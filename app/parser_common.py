...
_HEB = r"֐-׿"
# Run these in order; each strips a specific artifact.
_CLEANUP_RULES: list[tuple[re.Pattern, str]] = [
...
def clean(text: str) -> str:
    """Fix extraction artifacts: stray spaces inside Hebrew words and before punctuation."""
    prev = None
    cur = text
    while cur != prev:
        prev = cur
        for pattern, repl in _CLEANUP_RULES:
...
def trim_preamble(items: list[dict]) -> list[dict]:
    """Drop preamble items: titles, "by:" lines, cast descriptions, etc.

    Find the first dialogue item. The name immediately before it is the first
    real character name — everything before that name is preamble, *except* for
    any consecutive stage_direction items sitting directly between the name and
    the preamble (those set the scene and are kept).
    """
    first_dialogue = next((i for i, it in enumerate(items)
                           if it["type"] == "dialogue"), None)
    if first_dialogue is None or first_dialogue == 0:
        return items
    if items[first_dialogue - 1]["type"] != "name":
        return items  # unexpected structure — don't trim
    start = first_dialogue - 1
